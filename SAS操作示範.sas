...
%let sample_file = &home/sample_list.xlsx;
%let fin_file    = &home/financial_data.xlsx;
%let outdir      = &home;
...
/* ========== 1) 用 LIBNAME XLSX 匯入（避免 180-322） ========== */
libname x_sam xlsx "&sample_file";
libname x_fin xlsx "&fin_file";

/* 取得各檔的第一個工作表名稱（若你知道工作表名，可直接寫死） */
proc sql noprint;
  select memname into :sheet_sam trimmed
  from dictionary.tables
  where libname='X_SAM' and memtype='DATA'
  order by memname;

  select memname into :sheet_fin trimmed
  from dictionary.tables
  where libname='X_FIN' and memtype='DATA'
  order by memname;
quit;

/* 匯入成 WORK 資料集 */
data sample_list;    set x_sam.&sheet_sam; run;
data financial_data; set x_fin.&sheet_fin; run;

/* 釋放 libname（可留可不留） */
libname x_sam clear;
libname x_fin clear;

/* ========== 2) 合併基表（以 sample_list 為主；只取 b 端需要欄位，避免重複鍵值） ========== */
proc sql;
  create table merge_all_base as
  select
      a.companyID, a.companyName, a.year,
      /* 只取 b 端的財務欄位，不再取 b 的 companyID/companyName/year */
...
proc sql;
  ...
  select
    companyID,
    companyName,
    year,                                  /* 若 year 是字串，改寫成 year_n as year */
...
data merge_all;
  set merge_all;
  if cmiss(companyID) or missing(year) then delete; /* 鍵值 */
...
proc sort data=merge_all; by companyID year; run;
...
proc sort data=merge_partial; by year companyID; run;

/* ========== 7) 匯出 ========== */
proc export data=merge_all
  outfile="&outdir/merge_all.xlsx" dbms=xlsx replace;
  sheet="merge_all"; putnames=yes;
run;

proc export data=merge_partial
  outfile="&outdir/merge_partial.xlsx" dbms=xlsx replace;
  sheet="merge_partial"; putnames=yes;
run;
...
/* （可選）係數表另存 */
proc export data=reg_params outfile="&outdir/reg_params.xlsx" dbms=xlsx replace; run;